...
function on_btn_start_click(btn)
    ...
    if auto_is_running then
		...
		-- Tự đỡ khi bắt đầu
		nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\yBreaker_logic_skill", "StartParry")
        auto_run()
    end
end
...
function on_btn_cutskill_click(btn)
	...
	if form.btn_cutskill.Text == nx_function("ext_utf8_to_widestr", "Dừng Lại") then
		
		form.btn_cutskill.Text = nx_function("ext_utf8_to_widestr", "Cắt Chiêu")
		form.btn_cutskill.ForeColor = "255,255,255,255"
	else
		form.btn_cutskill.Text = nx_function("ext_utf8_to_widestr", "Dừng Lại")
		form.btn_cutskill.ForeColor = "255,220,20,60"
	end
end

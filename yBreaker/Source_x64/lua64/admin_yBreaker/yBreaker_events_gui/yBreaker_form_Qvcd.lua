...
local THIS_FORM = "admin_yBreaker\\yBreaker_form_Qvcd"
local pos =
{
		...
		if nx_is_valid(game_client) and nx_is_valid(game_visual) then 
			...
			if nx_is_valid(player_client) and nx_is_valid(game_player) and nx_is_valid(game_scence) then 
				...
				if form.check_time == 0 or os.difftime(os.time(), form.check_time) >= 50 then
					...
					local textchat = nx_value("gui").TextManager:GetFormatText(nx_string('<font color=\"#5FD00B\">{@1:text}</font> {@0:text}'), nx_widestr(nx_function("ext_utf8_to_widestr","Đang chạy nhiệm vụ Vô Cực Đảo")))
...
function DoQVCDOpen(form)
	...
	if SupportMap() then
		...
		tools_show_notice(nx_function("ext_utf8_to_widestr", "Chỉ hỗ trợ trên Vô Cực Đảo"))
		form.btn_control.Enabled = false
	end
	form.cbtn_q1.Checked = true
	form.cbtn_q2.Checked = true
	form.cbtn_q3.Checked = true
	form.cbtn_q4.Checked = true
	form.cbtn_q5.Checked = true
	form.cbtn_q6.Checked = true
	form.cbtn_q6.Checked = true
	form.cbtn_q7.Checked = true
	form.cbtn_q8.Checked = true
	form.cbtn_q9.Checked = true
	form.cbtn_q10.Checked = true
...
function CloseClick(btn)
	...
	if not form.auto_start then
		...
		local textchat = nx_value("gui").TextManager:GetFormatText(nx_string('<font color=\"#5FD00B\">{@3:text}</font> {@0:text} <font color=\"#F10F07\">{@1:text}</font> {@2:text}'), nx_widestr(nx_function("ext_utf8_to_widestr","Bấm ")), nx_widestr(nx_function("ext_utf8_to_widestr","Dừng")), nx_widestr(nx_function("ext_utf8_to_widestr","trước khi tắt")))

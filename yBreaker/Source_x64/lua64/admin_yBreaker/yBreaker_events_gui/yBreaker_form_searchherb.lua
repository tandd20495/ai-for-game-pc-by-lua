	...
	while auto_is_running == true do
		...
		if is_vaild_data == true then
		    ...
		    for i = 1, num_objs do
				...
				local txt_chat = nx_value("gui").TextManager:GetFormatText(nx_string('<font color="#EE0606">{@0:name}</font>(<a href="findpath,{@3:scene},{@4:x},{@5:y},{@6:z},{@7:ident}" style="HLStype1">{@1:x},{@2:z}</a>) - {@8:text}'), util_text(game_scence_objs[i]:QueryProp("ConfigID")), nx_widestr(herb_posX), nx_widestr(herb_posZ), nx_widestr(yBreaker_get_current_map()), nx_widestr(pathX), nx_widestr(pathY), nx_widestr(pathZ), nx_widestr(herb_ident), nx_widestr(nx_function("ext_utf8_to_widestr","Lụm lúa!")))
				
				-- Nếu có check vào SOS thì nháy màn hình và chat hệ thống
				if form.chk_sos.Checked then
					-- Flash game window
				...
				-- Hiện lên GUI
				form.mltbox_content:AddHtmlText(txt_chat, -1)
			...
			-- Type == 2: người
				if obj_type == 2 and nx_function("find_buffer", game_scence_objs[i], "buf_jzsj_040") then -- desc_buf_jzsj_040_0: buff cầm cây
					...
					if player_herb_name == player_name then
						...
						local txt_chat = nx_value("gui").TextManager:GetFormatText(nx_string("<font color=\"#ff2bdf\">{@0:name}</font> (<a href=\"findpath,{@3:scene},{@4:x},{@5:y},{@6:z},{@7:ident}\" style=\"HLStype1\">{@1:x},{@2:z}</a>) <font color=\"#EA2027\">{@8:bang}</font>"), nx_widestr(player_herb_name), nx_widestr(display_player_posX), nx_widestr(display_player_posZ), nx_widestr(yBreaker_get_current_map()), nx_widestr(pathX), nx_widestr(pathY), nx_widestr(pathZ), nx_widestr(player_herb_ident), player_herb_bang)

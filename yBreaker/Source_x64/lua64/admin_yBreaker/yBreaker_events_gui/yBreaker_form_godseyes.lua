...
function on_btn_control_click(btn)
    ...
    if auto_is_running then
        ...
        btn.Text = nx_function("ext_utf8_to_widestr", "Dừng Lại")
...
function reset_all_btns(skip)
    ...
    if skip ~= "" then
        auto_is_running = false
        form.btn_control.Text = nx_function("ext_utf8_to_widestr", "Tìm Cóc")
    ...
    if skip ~= "player" then
        auto_is_running_player = false
        form.btn_control_player.Text = nx_function("ext_utf8_to_widestr", "Tìm Người")

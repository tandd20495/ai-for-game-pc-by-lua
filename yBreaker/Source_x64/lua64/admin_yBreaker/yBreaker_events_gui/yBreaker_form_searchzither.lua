		...
		if is_vaild_data == true then
		    ...
		    for i = 1, num_objs do
		        ...
		        if obj_type == 2 and game_scence_objs[i]:FindProp("GameState") and  game_scence_objs[i]:QueryProp("GameState") == "QinGameModule" then
					...
					local txt_chat = nx_value("gui").TextManager:GetFormatText(nx_string('<font color="#EE0606">{@0:name}</font>(<a href="findpath,{@3:scene},{@4:x},{@5:y},{@6:z},{@7:ident}" style="HLStype1">{@1:x},{@2:z}</a>)<font color="#17FC0B">{@8:guildname}</font> - Team: <font color="#17FC0B">{@9:name}</font>'), nx_widestr(zither_name), nx_widestr(zither_posX), nx_widestr(zither_posZ), nx_widestr(yBreaker_get_current_map()), nx_widestr(pathX), nx_widestr(pathY), nx_widestr(pathZ), nx_widestr(zither_ident), nx_widestr(zither_bang), nx_widestr(zither_keypt))
					
					-- Nếu có check vào SOS

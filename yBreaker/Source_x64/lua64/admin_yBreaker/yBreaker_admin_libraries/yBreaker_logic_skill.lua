...
function AutoAttackBySet(set)
    if Running then
        ...
        Running = true
        loadConfig()
        nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\yBreaker_logic_item", "LoadPickItemData")
        loadSkillSet(set)
        doAutoAttack()
    end
end
...
function NgoiThien()
    StopFindPath()
    XuongNgua()
    if nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\yBreaker_logic_base", "GetLogicState") == 102 or isSwimming() then
        return
    end
...
function StopNgoiThien()
    if nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\yBreaker_logic_base", "GetRoleState") == "sitcross" then
...
function checkNearTarget(skill)
    ...
    if nx_is_valid(o) and not isDead(o) then
        if GetDistanceToObj(o) > r then
            --if nx_execute("logic_thien_the", "IsRunning") or nx_execute("logic_ltt", "IsRunning") then
...
function loopAttack()
    ...
    if RadiusModeFlg or NpcListModeFlg then
        local obj = nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\yBreaker_logic_base", "GetNearestObj", nx_current(), "isAttackingMeObj")
...
function isSwimming()
    local state = nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\yBreaker_logic_base", "GetRoleState")
...
function processPickItem()
    if nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\yBreaker_logic_item", "IsDroppickShowed") then
        nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\yBreaker_logic_item", "FlexPickItem")
        if nx_is_valid(LastPickNpc) then
            addToPickBlackList(LastPickNpc)
        end
        return true
    end
    local npc = nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\yBreaker_logic_base", "GetNearestObj", nx_current(), "isNeedPickNpc")
...
function findTargetObj()
    local npc = nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\yBreaker_logic_base", "GetNearestObj", nx_current(), "isNeedAttackNpc")
    if nx_is_valid(npc) then
        nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\yBreaker_logic_base", "SelectTarget", npc)
        return true
    end

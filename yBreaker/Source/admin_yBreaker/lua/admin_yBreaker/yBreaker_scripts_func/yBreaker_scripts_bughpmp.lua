...
function buff_full_hpmp()
    ...
    if not nx_is_valid(skill) then
        tools_show_notice(nx_function("ext_utf8_to_widestr", "Chưa học Lãm Tước Vĩ (cổ) chưa xài được chức năng này"), 2)
        return
    end
    ...
    -- Buff mana
    if nx_execute("admin_yBreaker\\yBreaker_admin_libraries\\tool_libs", "isAdmRights") and nx_is_valid(game_player) and get_buff_info("buf_CS_wd_tjq07") ~= nil then

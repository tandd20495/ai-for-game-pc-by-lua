...
local THIS_FORM = "admin_yBreaker\\yBreaker_form_modpvc"
...
function on_btn_usecard_click(btn)
    ...
    if SubType ~= -1 then
        ...
        if base_count ~= 2 then
            yBreaker_show_Utf8Text("Lỗi model: " .. nx_string(model))
            return
        end
        ...
        if currWeaponSubType == SubType then
            ...
            yBreaker_show_Utf8Text("Đã áp dụng Sau khi lưu thiết lập tự đổi set đồ và thay vũ khí sẽ có tác dụng")
        end
        return
    end
        ...
        if base_table ~= nil then
            ...
            if base_count == 2 then
                ...
                elseif prop_name == "Mount" then
                    ...
                    yBreaker_show_Utf8Text("Đã áp dụng loại Tọa Kỵ này. Nếu đang trên ngựa vui lòng xuống ngựa và gọi ngựa lại. Nếu chưa lên ngựa thì lên ngựa để áp dụng")
...
function updatePlayerShape()
    ...
    if sactive then
        -- Mặt Nạ
         -- Mũ áo quần giày   
         yBreaker_show_Utf8Text(nx_string(Cloth),10)
...
function on_btn_clear_hat_click(btn)
    unlinkPlayerSkin("hat")
    currentSuitSet.Hat = ""
    yBreaker_show_Utf8Text("Đã xóa tóc")
end
...
function on_btn_clear_cloth_click(btn)
    ...
    yBreaker_show_Utf8Text("Đã xóa áo")
end
...
function on_btn_clear_pants_click(btn)
    unlinkPlayerSkin("pants")
    currentSuitSet.Pants = ""
    yBreaker_show_Utf8Text("Đã xóa quần")
end
...
function on_btn_clear_shoes_click(btn)
    unlinkPlayerSkin("shoes")
    currentSuitSet.Shoes = ""
    yBreaker_show_Utf8Text("Đã xóa giày")
end
...
function on_btn_clear_cloak_click(btn)
    ...
    yBreaker_show_Utf8Text("Đã xóa phi phong")
end
...
function on_btn_clear_waist_click(btn)
    ...
    yBreaker_show_Utf8Text("Đã xóa trang sức đai")
end

-- Xóa Trang Sức Lưng
function on_btn_clear_back_click(btn)
    currentSuitSet.Back = ""
    yBreaker_show_Utf8Text("Đã bỏ trang sức lưng")
    local game_client = nx_value("game_client")
    local player_client = game_client:GetPlayer()
    local role = nx_value("role")
    local collect_card_manager = nx_value("CollectCardManager")
...
function on_btn_clear_face_click(btn)
    currentSuitSet.Face = ""
    yBreaker_show_Utf8Text("Đã bỏ mặt nạ")
    local game_visual = nx_value("game_visual")
    local game_player = game_visual:GetPlayer()
    local scene = nx_value("game_scene")
    local game_effect = scene.game_effect
...
function on_btn_clear_mount_click(btn)
    ...
    yBreaker_show_Utf8Text("Đã hủy thiết lập ngựa")
end

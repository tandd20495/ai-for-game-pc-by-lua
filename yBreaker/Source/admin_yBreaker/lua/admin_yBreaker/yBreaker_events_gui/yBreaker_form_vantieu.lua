...
local THIS_FORM = "admin_yBreaker\\yBreaker_form_vantieu"
...
function show_hide_form_vantieu()
	util_auto_show_hide_form(THIS_FORM)
end

-- Sửa ở form_stage_main\\form_main\\form_main_request (Thêm 3 hàm để lấy request)
-- ID Boss cướp tiêu xa
local arrayGuildBoss = {
    "guild_biaoche_boss_001",
    "guild_biaoche_boss_002",
    "guild_biaoche_boss_003",
    "guild_biaoche_boss_004",
    "guild_biaoche_boss_005",
    "guild_biaoche_boss_006",
    "guild_biaoche_boss_007",
    "guild_biaoche_boss_008",
    "guild_biaoche_boss_009",
    "guild_biaoche_boss_000",
    "guild_biaoche_boss_011",
    "guild_biaoche_boss_012",
    "guild_biaoche_boss_013",
    "guild_biaoche_boss_014",
    "guild_biaoche_boss_015",
    "guild_biaoche_boss_016",
    "guild_biaoche_boss_017",
    "guild_biaoche_boss_018",
    "guild_biaoche_boss_019",
    "guild_biaoche_boss_020"
}
local posBehindEscortNPC = nil
local timeAppearBoss = 0
            ...
            elseif escort_step == 4 then
                ...
                if objCheck == false or not nx_is_valid(objCheck) then
                    ...
                    escort_step = 5
							...
							local e_object = X_GetSpecEnemy("Resource","npc\\worldnpc1324")
                            local gboss = game_scence:GetSceneObj(nx_string(e_object.Ident))
                            local vgboss = game_visual:GetSceneObj(nx_string(e_object.Ident))
                    -- Kiểm tra xem có boss cướp tiêu không
                    --local gboss, vgboss = find_npc_to_kill(arrayGuildBoss, nil, nil, true)
                    if nx_is_valid(gboss) and nx_is_valid(vgboss) then
                        -- Set thời điểm boss ra
                        if timeAppearBoss == 0 then
                            timeAppearBoss = os.time()
                        end
                        form.mltbox_content:AddHtmlText(nx_function("ext_utf8_to_widestr", "Đánh cướp tiêu thí mẹ nó cái đã!!!"), -1)
                        -- Di chuyển về sau lưng cái xe tiêu cách 7 mét
                        if nx_is_valid(objCheck) then
                            if tools_difftime(timeAppearBoss) > 5 then
                                -- Xác định điểm đứng sau xe tiêu cách boss 7 mét từ giây thứ 6 trở đi
                                local dst = 0
                                local xx = objCheck.DestX
                                local zz = objCheck.DestZ
                                local radian = objCheck.DestOrient
                                local angle = radian_to_degree(radian)
                                while dst == 0 or getDistanceWithObj(posBehindEscortNPC, vgboss) < 8.5 do
                                    dst = dst + 1
                                    -- Sau lưng
                                    if angle <= 90 then
                                    ...
                                    posBehindEscortNPC = {xx, objCheck.DestY, zz}
                                end
                            end
                            if posBehindEscortNPC ~= nil and not tools_move_isArrived2D(posBehindEscortNPC[1], posBehindEscortNPC[2], posBehindEscortNPC[3]) then
                                -- Di chuyển về sau lưng xe tiêu cách boss 7 mét để đánh
                                if not is_moving() then
                                    tools_move(nil, posBehindEscortNPC[1], posBehindEscortNPC[2], posBehindEscortNPC[3], true, true)
                                end
                            elseif setTargetCompetitor(gboss) then
                                -- Đánh boss sau khi đã di chuyển ra xa hoặc lúc boss ra không quá 6s
                                killCompetitor(gboss, vgboss)
								if benefitFriends ~= nx_widestr("") then

        ...
        if is_vaild_data == true then
            ...
            if nx_is_valid(select_target) and nx_is_valid(select_target_visual) then
                ...
                if obj_type == TYPE_PLAYER then
                    ...
                    form.mltbox_content:AddHtmlText(nx_function("ext_utf8_to_widestr", "Đối thủ: ") .. nx_widestr(select_target:QueryProp("Name")), -1)
                else
                    -- Ghi tên đối thủ là NPC
                    form.mltbox_content:AddHtmlText(nx_function("ext_utf8_to_widestr", "Đối thủ: ") .. util_text(select_target:QueryProp("ConfigID")), -1)
                end
...
function auto_run_hgbc()
    ...
    -- Tọa độ vác đến đầu tiên chống lỗi map
    local toPos = {
        x = -0.268,
        y = 21.753,
        z = -5.806
    }
    -- Tọa độ nếu không tìm thấy cục đá thì dịch chuyển đến để tìm đá
    local posToFindBox = {
        x = -0.417,
        y = 21.753,
        z = -29.327
    ...
    -- Kiểm tra thực lực, nếu thực lực nhỏ hơn vô tiền khoáng hậu
    -- thì giảm cấp cục đá xuống
    if getPlayerPropInt("PowerLevel") < nx_int(91) then
        currentType = 3
        boxData[3][1] = 13060
        boxData[3][3] = "buf_6n_wyhs_bst05a"
        boxData[2][1] = 13055
        boxData[2][3] = "buf_6n_wyhs_bst02a"
        boxData[1][1] = 13050
        boxData[1][3] = "buf_6n_wyhs_bst01a"
    elseif getPlayerPropInt("PowerLevel") < nx_int(101) then
        currentType = 3
        boxData[3][1] = 13061
        boxData[3][3] = "buf_6n_wyhs_bst05b"
        boxData[2][1] = 13056
        boxData[2][3] = "buf_6n_wyhs_bst02b"
        boxData[1][1] = 13051
        boxData[1][3] = "buf_6n_wyhs_bst01b"
    elseif getPlayerPropInt("PowerLevel") < nx_int(111) then
        currentType = 4
        boxData[4][1] = 13066
        boxData[4][3] = "buf_6n_wyhs_bst010b"
        boxData[3][1] = 13062
        boxData[3][3] = "buf_6n_wyhs_bst05c"
        boxData[2][1] = 13057
        boxData[2][3] = "buf_6n_wyhs_bst02c"
        boxData[1][1] = 13052
        boxData[1][3] = "buf_6n_wyhs_bst01c"
    end
    ...
    while auto_is_running_hgbc == true do
        ...
        if is_vaild_data == true then
            ...
            if currentMap ~= map then
                logToForm("MAP này không phải MAP chạy nhiệm vụ. Vui lòng nhận Q, vào map rồi mới chạy auto.")
                reset_all_btns("----")
                return false
            end

            -- Nếu đang ôm cục đá thì di chuyển
            if get_buff_info(boxData[currentType][3]) ~= nil then
                logToForm("Đang ôm cục đá")
                local moveToPos = {}
                if game_player.PositionZ > (toPos.z - 0.8) or isAdmRights then
                    -- Di chuyển đến chỗ thả
                    -- Admin thì di chuyển trực tiếp đến luôn
                    moveToPos = dropPos
                else
                ...
                if not tools_move_isArrived(moveToPos.x, moveToPos.y, moveToPos.z, 0.8) then
                    if isAdmRights then
                        -- Admin thì nhảy trực tiếp
                        logToForm("Trực tiếp đến chỗ thả")
                        jump_to({moveToPos.x, moveToPos.y, moveToPos.z}, 85, 3)
                    else
                        -- Đưa camera quay về hướng cục đá
                        -- Di chuyển thủ công
                        logToForm("Dịch chuyển thủ công đến chỗ thả")
                        ...
                        if dropItent ~= nil then
                            ...
                            if nx_is_valid(visualobj) then
                                ...
                                if get_buff_info(boxData[currentType][3]) ~= nil then
                                    ...
                                    logToForm("Rớt mất tiêu cục đá rồi, đi lấy cục khác")
                                end
                            end
                        else
                            logToForm("Chưa tìm thấy chỗ vứt cục đá")
                        end
                    end
                end
            else
                logToForm("Đang rảnh tay")
                ...
                if objID ~= nil then
                    logToForm("Đã xác định được cục đá tiếp theo")
                    ...
                    if nx_is_valid(visualobj) then
                        ...
                        if not tools_move_isArrived(movePos[1], movePos[2], movePos[3], 0.8) then
                            if isAdmRights then
                                -- Admin thì nhảy trực tiếp
                                logToForm("Trực tiếp đến chỗ cục đá")
                                jump_to(movePos, 85, 3)
                            else
                                logToForm("Di chuyển thủ công đến cục đá")
                                -- Đưa camera quay về hướng cục đá
                            ...
                            if get_buff_info(buffCountDown) == nil then
                                logToForm("Đã đến nơi, đang ôm đá")
                                -- Nhấp cục đá
                                ...
                                logToForm("Đã đến nơi và đang chờ hết mệt để ôm đá tiếp")
                            end
                        end
                    end
                elseif not tools_move_isArrived(posToFindBox.x, posToFindBox.y, posToFindBox.z, 0.8) then
                    logToForm("Không thấy cục đá nào. Đang thử đi tìm cục đá")
                    if isAdmRights then
                        -- Admin thì nhảy trực tiếp
                        logToForm("Trực tiếp đến chỗ tìm đá")
                        jump_to({posToFindBox.x, posToFindBox.y, posToFindBox.z}, 85, 3)
                    else
                        logToForm("Di chuyển thủ công đến chỗ tìm đá")
                        -- Đưa camera quay về hướng cục đá
                        local angleY = getAngleForward(game_player.PositionX, game_player.PositionZ, posToFindBox.x, posToFindBox.z)
                        local scene = nx_value("game_scene")
                        local game_control = nx_execute("create_scene", "create_game_control", scene)
                        local camera_normal = game_control:GetCameraController(0)
                        camera_normal.CurYawAngle = angleY
                    ...
                    logToForm("Không thấy cục đá nào. Đứng chờ thử trong vô vọng")
                end
            end
        end
        nx_pause(0.2)
        ...
        if is_vaild_data == true then
            ...
            if step == 1 then
                ...
                if currentMap == MapQuest then
                    ...
                    form.mltbox_content:AddHtmlText(nx_function("ext_utf8_to_widestr", "Đang đi tìm NPC nhận nhiệm vụ"), -1)
                    -- Đến điểm nói chuyện với NPC
                    if not tools_move_isArrived(692.209, 4.294, 1691.107) then
                        if not is_moving() then
                            tools_move("city03", 692.209, 4.294, 1691.107, true)
                        end
                    elseif get_buff_info("buf_riding_01") ~= nil then
                        -- Xuống ngựa
                        nx_execute("custom_sender", "custom_remove_buffer", "buf_riding_01")
                        nx_pause(1)
                    elseif nx_is_valid(game_scence) then
                        -- Quét tìm NPC để nói chuyện
                        local game_scence_objs = game_scence:GetSceneObjList()
                        for i = 1, table.getn(game_scence_objs) do
                            if game_scence_objs[i]:FindProp("Type") and game_scence_objs[i]:QueryProp("Type") == 4 then
                                ...
                                if npc_id == receiveQuestNPC[1] then
                                    -- Mở cái bảng nói chuyện
                                    local is_talking = false
                                    while is_talking == false and auto_is_running_vptd do
                                        nx_execute("custom_sender", "custom_select", game_scence_objs[i].Ident)
                                        nx_pause(0.2)
                                        local form_talk = util_get_form("form_stage_main\\form_talk_movie", true)
                                        is_talking = form_talk.Visible

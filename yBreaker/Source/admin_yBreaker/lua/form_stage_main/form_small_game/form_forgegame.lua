--[[DO: Quest of hidden school Cổ Mộ Phái: Nuôi Ong Lấy Mật --]]
...
--[REM: Rem code is show dialog for yBreaker
    -- local gui = nx_value("gui")
...
function on_main_form_open(self)
  ...
  change_form_size()
    local form = nx_value("form_stage_main\\form_small_game\\form_forgegame")
    if nx_is_valid(form) then
        on_btn_startgame_click(form.btn_startgame)
    end
end
...
--[ADD: Play game for yBreaker
    yBreakerPlayGame(form)
--]
    return
  end
...
--[ADD: Function to playgame of quest for yBreaker
function yBreakerPlayGame(form)
    local btnName = "btnAction_" .. nx_string(currentAction)
    if not nx_find_custom(form, btnName) then
        return
    end
    local btn = form[btnName]

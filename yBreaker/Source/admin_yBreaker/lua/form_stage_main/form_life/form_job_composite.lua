...
--[ADD: Composite/Split for yBreaker
...
--[ADD: Handle event composite split button for yBreaker
function on_btn_composite_and_split_click(btn)	
	nx_execute(nx_current(),'autoCompositeSplit')
end
function autoCompositeSplit()
  local table_pos = {}
  local form = util_get_form("form_stage_main\\form_life\\form_job_composite", true)
	btn = form.btn_composite_and_split
	if btn.Text == nx_function("ext_utf8_to_widestr", "Chế|Phân Giải") then
		...
		local count = 0
		local result = iniformula:ReadString(sec_index, nx_string("ComposeResult"), "")
		...
		for index, view_item in pairs(viewobj_list) do
			table.insert(table_pos, nx_int(view_item.Ident))
		end
		while cas_status do
			nx_pause(1)
			if count < nx_number(num) then
				local game_client1 = nx_value("game_client")
				if not nx_is_valid(game_client1) then
					return
				end
				local view1 = game_client1:GetView(nx_string(121))
				if not nx_is_valid(view1) then
					return
				end
				local viewobj_list1 = view1:GetViewObjList()
				if table.getn(viewobj_list1) == table.getn(table_pos) then
					nx_execute("custom_sender", "custom_send_compose", nx_string(formula_id), nx_int(1), 0, 0)
				else
					...
					if nx_is_valid(form_split) then
						form_split.jobid = form.JobID
						nx_execute("custom_sender", "custom_lifeskill_split_item", form_split.jobid)
					end
					for index, view_item1 in pairs(viewobj_list1) do
						local view_item_obj = view_item1.Ident
						if not in_array(nx_int(view_item_obj), table_pos) then
							nx_execute("custom_sender", "custom_add_split_item", nx_int(121), nx_int(view_item_obj))
							nx_execute("custom_sender", "custom_split_item", nx_int(121), view_item1:QueryProp("UniqueID"))
							nx_pause(5.5)
							count = count + 1
						end
					end
	...
	if btn.Text == nx_function("ext_utf8_to_widestr", "Dừng") then
		btn.Text = nx_function("ext_utf8_to_widestr", "Chế|Phân Giải")
		cas_status = false
	end
end
--]
...
function show_composite_info(form, formula_id)
  ...
  form.ipt_1.MaxDigit = nx_int(999) --[Change number limit input value for composite/split ]--

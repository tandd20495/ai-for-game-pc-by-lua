--[[DO: Bỏ qua Q đập trứng sôi nổi yBreaker --]]
require("util_gui")
require("tips_data")
...
--[ADD: Set status win for whack egg for yBreaker
...
--[CHANGE: Set status win whack egg for yBreaker
    create_img_ani("win", gui.Desktop.Width / 2 - 868, gui.Desktop.Height / 5)
    play_sound("win")
--ADD:  game_visual:CustomSend(nx_int(CLIENT_CUSTOMMSG_SMAILL_GAME_MSG), nx_int(SMALLGAME_MSG_PRIZE_WHACK_EGG))
--]
...
function get_animation(data, state)
  local ani = ""
  if data >= FUQI_EGG and data <= NORMAL_EGG and 0 <= state and state <= 3 then

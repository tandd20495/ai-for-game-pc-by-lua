...
function Start()
    ...
    nx_execute("admin_zdn\\zdn_listener", "addListen", nx_current(), "30409", "onFullBoss", -1)
    nx_execute("admin_zdn\\zdn_listener", "addListen", nx_current(), "30361", "onFullBoss", -1)
    Running = true
    while Running do
        loopHaoKiet()
        nx_pause(0.2)
...
function Stop()
    ...
    StopFindPath()
    nx_execute("admin_zdn\\zdn_listener", "removeListen", nx_current(), "30409", "onFullBoss")
    nx_execute("admin_zdn\\zdn_listener", "removeListen", nx_current(), "30361", "onFullBoss")
...
function loopHaoKiet()
    ...
    if pickDeadBossItem() then
        local boss = nx_execute("admin_zdn\\zdn_logic_base", "GetNearestObj", nx_current(), "isBoss", "isNotInBlackList")
    ...
    XuongNgua()
    local target = nx_execute("admin_zdn\\zdn_logic_base", "GetNearestObj", nx_current(), "isNotDead", "isAttackable", "isInRange")
    ...
    nx_execute("admin_zdn\\zdn_logic_skill", "FlexAttackObj", target)
end
...
function loadSetting()
    local posStr = nx_string(IniReadUserConfig("HaoKiet", "Position", ""))
    if posStr ~= "" then
        local posList = util_split_string(posStr, ";")
...
function onFullBoss()
    ...
    local newResetTimeStr =
        ...
        Position.PosY,
        Position.PosZ,
        nx_execute("admin_zdn\\zdn_logic_base", "GetNextDayStartTimestamp")
    )
    ...
    return map ..
        "," ..
            nx_string(math.floor(x)) ..
                "," .. nx_string(math.floor(y)) .. "," .. nx_string(math.floor(z)) .. "," .. nx_string(time)
end
    ...
    if resetTimeStr ~= "" then
        ...
        for _, record in pairs(resetTime) do
            ...
            if
             ...
             then
                return nx_execute("admin_zdn\\zdn_logic_base", "GetCurrentDayStartTimestamp") >= nx_number(prop[5])
            end
        end
    end
    return true

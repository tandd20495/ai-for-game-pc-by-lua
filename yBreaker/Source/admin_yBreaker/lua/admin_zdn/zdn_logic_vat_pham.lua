...
function loopVatPham()
    ...
    for _, item in pairs(ItemList) do
        ...
        if item.itemId == "item_wqlk_01" then
            local hr = nx_execute("admin_zdn\\zdn_logic_base", "GetCurrentHour")
            if hr < 21 or hr > 22 then
                flg = false
            end
        end
        if flg and not nx_execute("admin_zdn\\zdn_logic_skill", "HaveBuff", item.buffId) then
...
function isItemInPickItemData(obj)
    ...
    for i = 1, cnt do
        ...
        if PickItemData[i] == util_text("admin_zdn\\zdn_item_vo_hoc") and nx_string(obj:QueryProp("ItemType")) == "23" then
            return true
        end
    end
    return false

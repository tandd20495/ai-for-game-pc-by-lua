...
local NPC_TU_LY_DONG = "newmp_sjy_lzy_rcrw003"
local TASK_INDEX = 73401
local NPC_TALK_FUNC_ID = 100073401
...
function doQuest()
    local obj = nx_execute("admin_zdn\\zdn_logic_base", "GetNearestObj", nx_current(), "isDaoTac")
    if nx_is_valid(obj) then
        nx_execute("admin_zdn\\zdn_logic_skill", "FlexAttackObj", obj)
        return
    end

    local npc = nx_execute("admin_zdn\\zdn_logic_base", "GetNearestObj", nx_current(), "isTuLyDong")
    if not nx_is_valid(npc) then
        GoToNpc(NPC_MAP, NPC_TU_LY_DONG)
        return
    end
...
function isTuLyDong(obj)
    return obj:QueryProp("ConfigID") == NPC_TU_LY_DONG
end

function isDaoTac(obj)
    return obj:QueryProp("ConfigID") == "newschool_lzy_sjy_ysjz001" and nx_number(obj:QueryProp("Dead")) ~= 1
end

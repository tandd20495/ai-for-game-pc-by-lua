...
local FinishTurn = 0
local MaxTurn = 20
local ByTurnFlg = false
local NeedProcessMail = true
...
function LoadConfig()
    ByTurnFlg = nx_string(IniReadUserConfig("ChienTruong", "ByTurn", "0")) == "1"
    if ByTurnFlg then
...
function loopChienTruong()
    ...
    if isDead(nx_execute("admin_zdn\\zdn_logic_base", "GetPlayer")) then
...
function prepareBattle()
    ...
    if ByTurnFlg and not CanRun() then
        Stop()
        return
    end
    registerBattle()
...
function goToCenter()
    ...
    nx_execute("admin_zdn\\zdn_listener", "addListen", nx_current(), "13000", "fixFindPath", 5)
    GoToPosition(1819.563477, -5.249885, 1475.334106)
end

function attackOtherPlayer()
    local enemy = nx_execute("admin_zdn\\zdn_logic_base", "GetNearestObj", nx_current(), "isNotDead", "isAttackable")
    if not nx_is_valid(enemy) then
        nx_execute("admin_zdn\\zdn_logic_skill", "PauseAttack")
        if needNgoiThien() then
        ...
        nx_execute("admin_zdn\\zdn_logic_skill", "StopNgoiThien")
        nx_execute("admin_zdn\\zdn_logic_skill", "FlexAttackObj", enemy)
    end
end
...
function isBattleFinish()
    local battleForm = nx_execute("admin_zdn\\zdn_logic_base", "GetChildForm", BATTLE_FORM_PATH)
    if nx_is_valid(battleForm) and battleForm.Visible then
        ...
        if player:QueryProp("BattlefieldState") ~= 3 or nx_function("find_buffer", player, "buff_battlefield_lock") then
            return true
        end
    end
    return false
...
function leaveBattle()
    ...
    --local str = nx_string(IniReadUserConfig("ChienTruong", "Turn", ""))
    --local turn = 0
    local ts = nx_execute("admin_zdn\\zdn_logic_base", "GetCurrentDayStartTimestamp")
    FinishTurn = FinishTurn + 1
    -- if str ~= "" then
    --     local prop = util_split_string(str, ",")
    --     if nx_number(prop[1]) == ts then
    --         turn = nx_number(prop[2])
    --     end
    -- end
    IniWriteUserConfig("ChienTruong", "Turn", nx_string(ts) .. "," .. nx_string(FinishTurn))
...
function isBeAttack()
    return nx_execute("admin_zdn\\zdn_logic_base", "GetLogicState") == 1
end

function processItem()
    local i = nx_execute("admin_zdn\\zdn_logic_vat_pham", "FindItemIndexFromVatPham", ITEM_DICH_CAN_DAN)
    if i ~= 0 and NeedProcessMail then
        nx_execute("admin_zdn\\zdn_listener", "addListen", nx_current(), "8014", "onDichCanDanMaxCapacity", 2)
        nx_execute("admin_zdn\\zdn_listener", "addListen", nx_current(), "power_redeem_4", "onDichCanDanMaxCapacity", 2)
        nx_execute("admin_zdn\\zdn_logic_vat_pham", "UseItem", 2, i)
        return true
    end

    i = nx_execute("admin_zdn\\zdn_logic_vat_pham", "FindItemIndexFromVatPham", ITEM_CHAN_KHI)
    if i ~= 0 then
        nx_execute("admin_zdn\\zdn_listener", "addListen", nx_current(), "8076", "deleteItemByConfig", 2, ITEM_CHAN_KHI)
        nx_execute("admin_zdn\\zdn_listener", "addListen", nx_current(), "8077", "deleteItemByConfig", 2, ITEM_CHAN_KHI)
        nx_execute("admin_zdn\\zdn_logic_vat_pham", "UseItem", 2, i)
        return true
    end

    i = nx_execute("admin_zdn\\zdn_logic_vat_pham", "FindItemIndexFromVatPham", ITEM_TU_VI)
    if i ~= 0 then
        nx_execute("admin_zdn\\zdn_logic_vat_pham", "UseItem", 2, i)
        return true
    end
    if not NeedProcessMail then
        return false
    end
    local boxI = nx_execute("admin_zdn\\zdn_logic_vat_pham", "FindItemIndexFromVatPham", ITEM_HOP_CAM_1)
    if boxI == 0 then
        boxI = nx_execute("admin_zdn\\zdn_logic_vat_pham", "FindItemIndexFromVatPham", ITEM_HOP_CAM_2)
    end
    if boxI == 0 then
        boxI = nx_execute("admin_zdn\\zdn_logic_vat_pham", "FindItemIndexFromVatPham", "haiwai_yn_xyyb_01")
    end
    if boxI == 0 then
        boxI = nx_execute("admin_zdn\\zdn_logic_vat_pham", "FindItemIndexFromVatPham", "haiwai_yn_xyyb_02")
    end
    if boxI == 0 then
        boxI = nx_execute("admin_zdn\\zdn_logic_vat_pham", "FindItemIndexFromVatPham", "haiwai_yn_xyyb_03")
    end
    if boxI == 0 then
        return false
    end
    if nx_execute("admin_zdn\\zdn_logic_vat_pham", "IsDroppickShowed") then
        ...
        nx_execute("admin_zdn\\zdn_logic_vat_pham", "UseItem", 2, boxI)
    end
    return true
end
...
function processMail()
    if ByTurnFlg and not NeedProcessMail then
        return false
    end
    local client = nx_value("game_client")
    local player = client:GetPlayer()
...
function onDichCanDanMaxCapacity()
    IniWriteUserConfig("ChienTruong", "ResetTime", nx_execute("admin_zdn\\zdn_logic_base", "GetNextDayStartTimestamp"))
    if ByTurnFlg then
        NeedProcessMail = false
    else
        Stop()
    end
end
...
function deleteItemByConfig(config)
    local i = nx_execute("admin_zdn\\zdn_logic_vat_pham", "FindItemIndexFromVatPham", config)
    if i ~= 0 then
...
function needNgoiThien()
    ...
    if
        ((hpRatio >= 74 and nx_execute("admin_zdn\\zdn_logic_skill", "HaveBuff", "buf_baosd_01")) or hpRatio >= 94) and
            mpRatio >= 93
     then
        return false
    end

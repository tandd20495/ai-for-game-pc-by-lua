...
local function jumpTo(x, y, z)
	if nx_execute("admin_zdn\\zdn_logic_thien_the", "IsRunning") then
		return
	end
	if nx_execute("admin_zdn\\zdn_logic_ltt", "IsRunning") then
		return
	end
	local role = nx_value("role")
...
local function shortJumpCollide()
	if not nx_execute("admin_zdn\\zdn_logic_on_tuyen", "IsRunning") then
		XuongNgua()
	end
	local role = nx_value("role")
	if not nx_is_valid(role) then
...
local function setFindPath(x, y, z)
	...
	if nx_execute("admin_zdn\\zdn_logic_base", "GetLogicState") == 102 then
		endSitCross()
		return false
	end
	pathFind(x, y, z)

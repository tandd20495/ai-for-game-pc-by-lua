...
function startQuest()
    ...
    -- tim npc
    local npc1 = nx_execute("admin_zdn\\zdn_logic_base", "GetNearestObj", nx_current(), "isFirstQuestNpc")
    ...
    local npc2 = nx_execute("admin_zdn\\zdn_logic_base", "GetNearestObj", nx_current(), "isSecondQuestNpc")
...
function isPilesShowed()
    local piles = nx_execute("admin_zdn\\zdn_logic_base", "GetNearestObj", nx_current(), "isPileNpc")
...
function isBossShowed()
    local boss = nx_execute("admin_zdn\\zdn_logic_base", "GetNearestObj", nx_current(), "isSceneBossNpc")
...
function getBuffAndReturnToBoss()
    if not nx_execute("admin_zdn\\zdn_logic_skill", "HaveBuffPrefix", "buf_6n_cc_sxsl_one_") then
        getBuffFromPiles()
        return
    end
...
function exitQuestScene()
    local npc = nx_execute("admin_zdn\\zdn_logic_base", "GetNearestObj", nx_current(), "isSecondSceneNpc")
    if not nx_is_valid(npc) then
        return
    end
    if GetDistanceToObj(npc) > 2 then

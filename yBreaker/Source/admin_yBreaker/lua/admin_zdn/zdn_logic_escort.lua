...
function loadEscortData()
    ...
    local escortInfo = nx_resource_path() .. "yBreaker\\data\\escort_info.ini"
...
function isShipping()
    return nx_execute("admin_zdn\\zdn_logic_skill", "HaveBuff", "buff_yunbiao_escortbuff")
end
...
function beAttacking()
    return nx_execute("admin_zdn\\zdn_logic_skill", "HaveBuffPrefix", "buf_guild_biaoche_shp")
end
...
function attackNpcAttacker(target)
    ...
    elseif currentSkill == "skill_guild_biaoche_boss_001" then
        if TimerDiff(target.TaoluTimer) > 2 then
            nx_execute("admin_zdn\\zdn_logic_skill", "Fly")
            nx_execute("admin_zdn\\zdn_logic_skill", "StartParry")
        end
    else

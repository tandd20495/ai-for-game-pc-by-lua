...
function Log(txt)
	local content =
		nx_widestr("[") ..
		nx_widestr(nx_execute("admin_zdn\\zdn_logic_base", "GetCurrentHourHuman")) .. nx_widestr("] ") .. nx_widestr(txt)
...
function onBindTrigger()
	...
	local content =
		nx_widestr("[") ..
		nx_widestr(nx_execute("admin_zdn\\zdn_logic_base", "GetCurrentHourHuman")) .. nx_widestr("] ") .. nx_widestr(r.state)
	...
	if r.state == "path_finding" then
		nx_execute("admin_zdn\\zdn_logic_vat_pham", "UseItem", 2, 69)
	end
end

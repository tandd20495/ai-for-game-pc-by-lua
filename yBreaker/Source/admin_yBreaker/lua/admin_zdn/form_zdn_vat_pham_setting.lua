...
function onBtnAddItemClick()
	...
	util_show_form("admin_zdn\\form_zdn_select_buff", true)
	nx_execute("admin_zdn\\form_zdn_select_buff", "SetItem", itemId)
end
...
function addRowToItemGrid(item)
	...
	local itemPhoto = createImageControl(item.itemId, nx_execute("admin_zdn\\zdn_logic_vat_pham", "GetItemPhoto", item.itemId))
...
function loadNoiTuData()
	NoiTuData = IniLoadAllData(nx_resource_path() .. "yBreaker\\data\\vatpham.ini")
end
...
function processNoiTuItem(itemId)
	for _, detail in pairs(NoiTuData) do
		if detail.item == itemId then
			...
			DoAddItem(itemId, buffId, buffId, nx_execute("admin_zdn\\zdn_logic_vat_pham", "GetItemPhoto", itemId))
			return true
		end
	end
	return false

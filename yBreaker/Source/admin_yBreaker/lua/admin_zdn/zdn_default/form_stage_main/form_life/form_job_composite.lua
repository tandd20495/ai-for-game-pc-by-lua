...
function show_composite_info(form, formula_id)
    ...
    if GameModule == "BrokenDoorModule" and TableNPCLimit == "" then
        ...
        if GameModule ~= "" and Profession ~= "sh_ss" and Profession ~= "sh_hs" then
    ...
    local logic = "admin_zdn\\zdn_logic_nghe"
    if nx_execute(logic, "IsRunning") then
        form.btn_composite.Text = Utf8ToWstr("Dừng")
        form.btn_composite.ForeColor = "255,220,20,60"
    end
end

function on_btn_composite_click(btn)
    local form = btn.ParentForm
    local zdnNum = nx_number(form.ipt_1.Text)
    --
    if SelectedProfession == "sh_ss" or SelectedProfession == "sh_hs" then
        local logic = "admin_zdn\\zdn_logic_nghe"
        if nx_execute(logic, "IsRunning") then
            nx_execute(logic, "Stop")
        else
            nx_execute(
                "admin_zdn\\zdn_event_manager",
                "Subscribe",
                logic,
                "on-task-stop",
                nx_current(),
            ...
            form.btn_composite.Text = Utf8ToWstr("Dừng")
            form.btn_composite.ForeColor = "255,220,20,60"
            nx_execute(logic, "Start", SelectedProfession, form.CurFormulaID, zdnNum)
        end
        return
    end
    --
...
function onZdnNgheStop(_, profession)
    local form = nx_value("form_stage_main\\form_life\\form_job_composite")
    if not nx_is_valid(form) or not nx_find_custom(form, "btn_composite") then
        return
    end
    if Utf8ToWstr("Dừng") == form.btn_composite.Text then
        form.btn_composite.Text = nx_widestr(life_btn_name_table[nx_string(profession)][1])
        form.btn_composite.ForeColor = "255,255,255,255"
    end
end

...
local THIS_FORM = 'auto_new\\form_auto_khd'
		...
		for i = 1, table.getn(view_table) do
			...
			if view.Ident == nx_string('80') then
				...
				for k = 1, table.getn(view_obj_table) do
					...
					local list = isPickList(nx_string(file_name),nx_string(key))	
					if list ~= nil then
...
local item_select = {	
	...
	'item_exc_fc_mml',
	'box_xmp_fc_01',
	'hcitem_1000053', -- Mặc Tử Bi Ti [Gian]
	'hcitem_1000054', -- Mặc Tử Bi Ti [Vĩ]
	'hcitem_1000056', -- Dương Xuân Gian Tấu
	...
	'box_fc_hss_001', -- Hoang Thú Thạch cẩm hạp 1 - 10
	'box_fc_hss_002',
	'box_fc_hss_003',
	'box_fc_hss_004',
	'box_fc_hss_005',
	...
	'box_fc_hss_009',
	'box_fc_hss_010',
	'box_fc_mml_001', -- Ma môn lệnh cẩm hạp 1 - 10
	'box_fc_mml_002',
	'box_fc_mml_003',
	'box_fc_mml_004',
	'box_fc_mml_005',
...
local item_select1 = "item_exc_fc_hss,item_exc_fc_mml,box_xmp_fc_01,hcitem_1000053, hcitem_1000054,hcitem_1000056,jm_hcitem_tj_saxe_03,jm_hcitem_tj_saxe_04,hcitem_tj_gun_03,hcitem_tj_gun_04,jm_hcitem_tj_token_03,jm_hcitem_tj_token_04,drug_1005,toxicant_1005,box_fc_hss_001,box_fc_hss_002,box_fc_hss_003,box_fc_hss_004,box_fc_hss_005,box_fc_hss_006,box_fc_hss_007,box_fc_hss_008,box_fc_hss_009,box_fc_hss_010,box_fc_mml_001,box_fc_mml_002,box_fc_mml_003,box_fc_mml_004,box_fc_mml_005,box_fc_mml_006,box_fc_mml_007,box_fc_mml_008,box_fc_mml_009,box_fc_mml_010"
...
function btn_start_tdc(btn)
	...
	if nx_running(nx_current(),'auto_clone_khd') then
		auto_khd = false
		nx_execute(nx_current(),'setAutoKHDState',auto_khd)
		nx_kill(nx_current(),'auto_clone_khd')
	else
		auto_khd = true
		nx_execute(nx_current(),'setAutoKHDState',auto_khd)
		nx_execute(nx_current(),'setStagePausePlay',true)
		nx_execute(nx_current(),'auto_clone_khd')
	end
	updateBtnAutoPause(form)
	updateBtnAuto(form)
end
updateCheckedAuto = function(form)
	local songthich = getIni_Nc6('auto_khd.ini','String','Setting_TDC','songthich', '')
	local isDicot = getIni_Nc6('auto_khd.ini','String','Setting_TDC','ntlt', '')
	local casot = getIni_Nc6('auto_khd.ini','String','Setting_TDC','casot', '')
	local lagbdh = getIni_Nc6('auto_khd.ini','String','Setting_TDC','lagbdh', '')
	local isUseKN =  getIni_Nc6('auto_khd.ini','String','Setting_TDC','linhnguyen', '')
	local isUseLN = getIni_Nc6('auto_khd.ini','String','Setting_TDC','kimnguyen', '')
	local lam = getIni_Nc6('auto_khd.ini','String','Setting_TDC','lam', '')
...
function on_cbtn_auto_box_6(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','lam','true')		
	else
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','lam','false')
	end
end
function is_list_add(inifile, content)
	local form = nx_value(THIS_FORM)
	for k = 1, table.getn(list_pickup) do
		if list_pickup[k] ~= '' and list_pickup[k] == content then		   return true		
		end
	end	
	return false
end
function is_list_del(inifile, content)
	...
	for k = 1, table.getn(list_pickup) do
		if list_pickup[k] ~= '' and list_pickup[k] == content then
			...
			local file = add_file_tdc(inifile)
			...
			ini.FileName = file
			if ini:LoadFromFile() then
				ini:WriteString(nx_string('Setting_TDC'), 'listnhat', nx_string(form.itemString))				
				ini:SaveToFile()
			end
			return true		
		end
...
function btn_start_del(btn)
	local form = btn.ParentForm	
	local file1 = add_file_tdc('auto_khd.ini')
	if not nx_function('ext_is_file_exist', file1) then	
	 ...
	 if string.len(nx_string(form.cbx_input_item_skip_auto.Text)) > 0 and form.cbx_input_item_skip_auto.Text ~= '' and form.cbx_input_item_skip_auto.Text ~= nil then
        ...
        if is_list_del('auto_khd.ini', x) then  
...
function btn_start_add(btn)
	...
	if nx_is_valid(form) then
		...
		if is_list_add('auto_khd.ini', item_name) then
			...
			local file = add_file_tdc('auto_khd.ini')
			if not nx_function('ext_is_file_exist', file) then 
				create_config()
			end
			local ini = nx_create('IniDocument')
...
function loadItemList(form)
	...
	ini.FileName = file
	if ini:LoadFromFile() then		
		form.itemString = ini:ReadString(nx_string('Setting_TDC'), 'listnhat', '')
...
function on_cbtn_auto_box_7(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','songthich','true')		
	else
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','songthich','false')
	end
end
function on_cbtn_auto_box_1(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','ntlt','true')		
	else
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','ntlt','false')
	end
end
function on_cbtn_auto_box_2(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','casot','true')		
	else
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','casot','false')
	end
end
function on_cbtn_auto_box_3(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','lagbdh','true')		
	else
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','lagbdh','false')
	end
end
function on_cbtn_auto_box_4(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','linhnguyen','true')		
	else
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','linhnguyen','false')
	end
end
function on_cbtn_auto_box_5(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','kimnguyen','true')		
	else
		SaveSetting_nc6('auto_khd.ini','Setting_TDC','String','kimnguyen','false')
	end
end

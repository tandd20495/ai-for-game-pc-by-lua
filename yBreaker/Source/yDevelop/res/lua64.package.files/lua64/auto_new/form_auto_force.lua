...
function load_tiguan_one(form)
	...
	local new_school = get_school_force()
	...
	for i = 1, keys_count do		
		local query_school = ini:GetSectionItemValue(sec_index,i-1)
		--wstrToUtf8(readIni(file_data,new_school,i,''))
...
function on_btn_close_click(form)   
  local form = nx_value("auto_new\\form_auto_force")
...
function on_cbx_input_sh_selected(cbx)
	...
	for i = 1, keys_count do		
		...
		if split_item[2] ~= nil then
			if form.cbx_input_sh.InputEdit.Text == utf8ToWstr(split_item[2]) then
				form.qn_task = split_item[1]
				form.title = utf8ToWstr(split_item[2])
			end
		end
	end	
end
function btn_add_start(btn)
	...
	if ti_today_status then
		nx_execute(nx_current(),'exe_auto_tun_ti_today')
		start_ti_td = false
		if nx_running(nx_current(),'exe_auto_tun_ti_today') then
			nx_kill(nx_current(),'exe_auto_tun_ti_today')
		end
	else
		nx_execute(nx_current(),'exe_auto_tun_ti_today')		
	end	

...
local THIS_FORM = 'auto_new\\form_auto_spy'
...
function btn_auto_start(btn)
	...
	if auto_running_full('auto_new\\form_auto_ai_new','exe_auto_sp_state_only') then
		showUtf8Text(auto_please_wait_stop)
		auto_execute_full('auto_new\\form_auto_ai_new','turn_off_auto_sp',false)
		click_mouse_fast = 0
	else		
		...
		auto_execute_full('auto_new\\form_auto_ai_new','exe_auto_sp_only')		
		nx_pause(1)		
	end	
	updateBtnAuto()
end
function updateBtnAuto()
	local form = nx_value("auto_new\\form_auto_spy")

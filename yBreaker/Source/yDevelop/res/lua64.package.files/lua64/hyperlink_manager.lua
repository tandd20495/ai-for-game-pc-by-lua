...
function find_path_npc_item(strData, click)
 ...
 elseif 0 <= nx_function("ext_ws_find", wstr_data, nx_widestr("findObj")) then
    ...
    local y = nx_int(param_table[4])
    local z = nx_int(param_table[5])
    local text = nx_string(param_table[6])
	local game_client = nx_value("game_client")
	local client_scene = game_client:GetSceneObj(text)
		if nx_is_valid(client_scene) then
			x,y,z = client_scene.DestX, client_scene.DestY, client_scene.DestZ
			nx_execute("custom_sender", "custom_select", text)
		end
		path_finding:FindPathScene(scene_id, x, y, z, 0)
	end
    ...
    if count >= 3 then
      ...
      if -10000 < nx_number(x) then
        if trace_flag == 1 then
          ...
          find_npc_z = z
          find_npc = true
            local jump_fast = nx_execute('auto_new\\auto_script','getRunBLinkState')
          if jump_fast then
            if nx_execute('auto_new\\autocack','get_current_map') == scene_id then
              nx_execute('auto_new\\autocack','jump_to_fast',x,y,z,scene_id)
            else
              nx_execute('auto_new\\autocack','showUtf8Text','This Map is not npc map')
            end  
          else

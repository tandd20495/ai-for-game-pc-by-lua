...
function init_ui_content(form)	
  if nx_execute("form_stage_main\\form_life\\form_job_main_new", "find_life_job", "sh_hs") then
...
function loadMusic()
	...
	local sec_index = job_info_ini:FindSectionIndex('sh_hs')
...
local FORM_PIC = "form_stage_main\\form_small_game\\form_game_picture"
function btn_start_music(cbtn)
	...
	while form.auto_start do
		...
		if nx_is_valid(game_client)and nx_is_valid(game_visual)then
			...
			local form_pic = nx_value("form_stage_main\\form_small_game\\form_game_picture")
			...
			if logicstate ~= 10 and logicstate ~= 102 and logicstate ~= 121 and logicstate ~= 120 and player_client:QueryProp("GameState") ~= "QinGameModule" then
				if auto_music_id_list ~= nil and table.getn(auto_music_id_list) > 0 then
					if not nx_is_valid(form_pic) then
						...
						for i = 1 , table.getn(get_item) do
							...
							if nx_number(amount) < nx_number(amount_get) then 
								autoBuyItemJob(nx_string('Shop_hs_05001'),nx_string(set_item[1]),nx_number(amount_get),false)
							end
						end	
						nx_pause(5)
						...
						if nx_is_valid(PictureGame) then
							local form_hand = nx_value("form_stage_main\\form_small_game\\form_game_picture")
							nx_pause(1)
							if nx_is_valid(form_hand) and form_hand.btn_start.Visible then			 	
								on_btn_pic_click()					
							end
						end	
					end
...
function on_btn_pic_click(form)
  local form = nx_value(FORM_PIC)
  ...
  nx_pause(1)
  nx_execute("form_stage_main\\form_small_game\\form_game_picture",'sapXep',form)
  --timer:Register(200, -1, nx_current(), "on_update_pic", form, -1, -1)
end
function on_update_pic(form)
  local form = nx_value(FORM_PIC)
  if nx_is_valid(nx_value(FORM_PIC)) then
  end
  local count = 0
  local grid = form.imagegrid_pic
  for i = 1, form.grid_row_num do
    for j = 1, form.grid_clomn_num do
      local images = grid:GetItemImage(count)
      local num_find = "_" .. needtofind .. ".png"
      images = string.gsub(images, "gui\\mainform\\smallgame\\picture_game\\picture\\", "")
      if string.find(images, num_find) ~= nil then
        if count ~= needtofind - 1 then
          local PictureGame = nx_value("PictureGame")
          if not nx_is_valid(PictureGame) then
            return
          end
          nx_execute("form_stage_main\\form_small_game\\form_game_picture", "on_imagegrid_pic_select_changed", grid, count)
          nx_pause(0.5)
          nx_execute("form_stage_main\\form_small_game\\form_game_picture", "on_imagegrid_pic_select_changed", grid, needtofind - 1)
        end
        if needtofind >= form.grid_row_num * form.grid_clomn_num then
          needtofind = 1
        else
          needtofind = needtofind + 1
        end
      end
      count = count + 1
    end
...
function showHideForm()
	util_show_form('auto_new\\form_auto_job_hs', true)
end
...
function update_btn_start_stop()
	local form = util_get_form("auto_new\\form_auto_job_hs", false, false)

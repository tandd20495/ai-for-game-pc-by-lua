...
local THIS_FORM = "auto_new\\form_auto_homepoint"
...
function on_tree_hp_select_changed(self)
    ...
    if node.IsHomePoint and node.id ~= "" then
        ...
        if not IsExists then
            ...
            if Max1 < 2 then
                showUtf8Text(AUTO_LOG_HOMEPOINT1, 2)
                IsBusy = false
                return false
            end
            ...
            if CountJiangHuHomePoint > 1 and LastHomePoint ~= "" then
                showUtf8Text(AUTO_LOG_DELETE_HOMEPOINT .. wstrToUtf8(LastHomePointText))
                send_homepoint_msg_to_server(3, LastHomePoint) -- 3: Xóa điểm dừng chân
                nx_pause(0.5)
            end
            local typename, htext = get_type_homepoint(hp)
            showUtf8Text(AUTO_LOG_ADD_NEW_HOMEPOINT.. wstrToUtf8(util_text(nx_string(htext))))		

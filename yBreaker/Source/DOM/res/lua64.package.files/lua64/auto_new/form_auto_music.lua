			...
			for k = table.getn(formula_list), 1, -1 do
				...
				if is_Learned_formula(formula_id) then
					...
					if m_info == "musician_10053" or m_info == "musician_10061" or m_info == "musician_10054" then
						table.insert(auto_music_id_list, 1, {id = formula_id, name = name})
					else
						table.insert(auto_music_id_list, {id = formula_id, name = name})
					end
				end
			end
		end
...
function btn_start_music(cbtn)
	...
	while form.auto_start do
		...
		if nx_is_valid(game_client)and nx_is_valid(game_visual)then
			...
			local form_music = nx_value("form_stage_main\\form_small_game\\form_mini_qingame")	
			if nx_is_valid(form_music) then
				form_music.t_speed = 0.01
			end
			if logicstate ~= 10 and logicstate ~= 102 and logicstate ~= 121 and logicstate ~= 120 and player_client:QueryProp("GameState") ~= "QinGameModule" then
				if auto_music_id_list ~= nil and table.getn(auto_music_id_list) > 0 then					
				  nx_execute("custom_sender", "custom_doqin", auto_music_id_list[nx_number(form.cbx_music_load.DropListBox.SelectIndex) + 1].id)
...
function showHideForm()
	util_show_form('auto_new\\form_auto_music', true)
end
...
function update_btn_start_stop()
	local form = util_get_form("auto_new\\form_auto_music", false, false)

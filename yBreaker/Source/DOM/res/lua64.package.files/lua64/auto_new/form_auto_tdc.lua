...
local THIS_FORM = 'auto_new\\form_auto_tdc'
...
function btn_start_tdc(btn)
	...
	if nx_running(nx_current(),'auto_tdc_don') then
		nx_execute(nx_current(),'setAutoTDCState',false)
		nx_kill(nx_current(),'auto_tdc_don')
	else
		nx_execute(nx_current(),'setAutoTDCState',true)
		nx_execute(nx_current(),'setStagePausePlay',true)
		nx_execute(nx_current(),'auto_tdc_don')
	end
	updateBtnAutoPause(form)
	updateBtnAuto(form)
end
updateCheckedAuto = function(form)
	local songthich = getIni_Nc6('auto_tdc.ini','String','Setting_TDC','songthich', '')
	local isDicot = getIni_Nc6('auto_tdc.ini','String','Setting_TDC','ntlt', '')
	local casot = getIni_Nc6('auto_tdc.ini','String','Setting_TDC','casot', '')
	local lagbdh = getIni_Nc6('auto_tdc.ini','String','Setting_TDC','lagbdh', '')
	local isUseKN =  getIni_Nc6('auto_tdc.ini','String','Setting_TDC','linhnguyen', '')
	local isUseLN = getIni_Nc6('auto_tdc.ini','String','Setting_TDC','kimnguyen', '')
	local lam = getIni_Nc6('auto_tdc.ini','String','Setting_TDC','lam', '')
...
function on_cbtn_auto_box_6(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','lam','true')		
	else
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','lam','false')
	end
end
...
function btn_start_del(btn)
	local form = btn.ParentForm	
	local file1 = add_file_tdc('auto_tdc.ini')
	if not nx_function('ext_is_file_exist', file1) then	
	 ...
	 if string.len(nx_string(form.cbx_input_item_skip_auto.Text)) > 0 and form.cbx_input_item_skip_auto.Text ~= '' and form.cbx_input_item_skip_auto.Text ~= nil then
        ...
        if is_list_del('auto_tdc.ini', x) then  
...
function add_item_cbx_auto(form,item_config_id)
	...
	if is_list_add('auto_tdc.ini', item_name) then
		...
		local file = add_file_tdc('auto_tdc.ini')
		if not nx_function('ext_is_file_exist', file) then 
			create_config()
		end
		local ini = nx_create('IniDocument')
...
function on_cbtn_auto_box_7(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','songthich','true')		
	else
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','songthich','false')
	end
end
function on_cbtn_auto_box_1(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','ntlt','true')		
	else
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','ntlt','false')
	end
end
function on_cbtn_auto_box_2(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','casot','true')		
	else
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','casot','false')
	end
end
function on_cbtn_auto_box_3(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','lagbdh','true')		
	else
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','lagbdh','false')
	end
end
function on_cbtn_auto_box_4(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','linhnguyen','true')		
	else
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','linhnguyen','false')
	end
end
function on_cbtn_auto_box_5(cbtn)	
	if cbtn.Checked then
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','kimnguyen','true')		
	else
		SaveSetting_nc6('auto_tdc.ini','Setting_TDC','String','kimnguyen','false')
	end
end

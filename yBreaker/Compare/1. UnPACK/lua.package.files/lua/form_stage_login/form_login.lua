...
load_account_by_dir = function(form)
	local file_ini = nx_execute('auto_new\\autocack','add_file_res','list_id')
	...
	if ini:LoadFromFile() then	
		if ini:FindSection('list_acc') then
			local list_acc = wstrToUtf8(nx_execute('auto_new\\autocack','readIni',file_ini,'list_acc','list_id',''))			
			...
			if table.getn(count_pos) > 0 then
				for j = 1,table.getn(count_pos) do
					...
					local del = create_multitextbox('form_stage_login\\form_login', 'on_click_btn_del','Del',count_pos[j], 'HLStypebargaining')
...
btn_add_start = function(btn)
	...
	nx_execute('auto_new\\autocack','writeIni',file_ini,'list_acc','list_id',list_id)
...
on_click_btn_del = function(btn)
	...
	nx_execute('auto_new\\autocack','writeIni',file_ini,'list_acc','list_id',data_item)
	load_account_by_dir(form)
end
...
load_data_auto_cack = function()
	if not check_load then
		nx_execute('auto_new\\autocack','auto_cack','0')
		nx_execute('auto_new\\autocack','auto_cack','2')
		nx_execute('auto_new\\autocack','auto_cack','5')		
		nx_execute('auto_new\\form_auto_encrypt','auto_cack','0')	
		 nx_execute('auto_new\\form_auto_hotkey','auto_cack','5')
		nx_execute('auto_new\\form_auto_run_func','auto_cack','1')
		nx_execute('auto_new\\form_auto_run_func','auto_cack','2')		
		nx_execute('auto_new\\form_auto_skill','auto_cack','0')
		 nx_execute('auto_new\\form_auto_skill','auto_cack','6')	
		nx_execute('auto_new\\form_auto_swap_old','auto_cack','0')
		nx_execute('auto_new\\form_auto_swap_old','auto_cack','2')	
		nx_execute('auto_new\\auto_script','auto_cack','2')	
		check_load = true
	end
end
init_auto_cack = function()
	...
	if nx_running('auto_new\\form_auto_ai_new', 'get_start_ai_status') then
		...
		nx_kill('auto_new\\form_auto_ai_new', 'off_ai_auto')
	end
end
...
function on_btn_enter_click(self)
  ...
  if password == "" and "" == login_validate then
	local check = nx_execute('auto_new\\form_auto_encrypt','get_unlock_pass_1',account) --set_pass_login
	nx_pause(0.5)
	if check ~= nil then
		form.ipt_2.Text = nx_widestr("")
		form.ipt_2:Append(nx_function("ext_utf8_to_widestr",check))	
		nx_pause(0.5)	
		password = nx_string(form.ipt_2.Text)		
	else

...
function LoadSetting(form)
	...
	local account = game_config.login_account
	local dir = nx_function("ext_get_current_exe_path") .. "ht_auto_" .. account 
	local file = dir .. nx_string("\\auto_donghai.txt")
...
function LoadSetting2(form)
	local game_config = nx_value("game_config")
	local account = game_config.login_account
	local dir = nx_function("ext_get_current_exe_path") .. "ht_auto_" .. account 
	local file = dir .. nx_string("\\auto_donghai_danger.txt")
	local gm_list = nx_create("StringList")
	if not gm_list:LoadFromFile(file) then
		nx_destroy(gm_list)
		return 0
	...
	for i = 0, gm_num - 1 do
		...
		if gm ~= "" then
			local gui = nx_value("gui")
			if nx_is_valid(gui) then
				form.combobox2.DropListBox:AddString(nx_widestr(gm))
			end
			item2[i+1] = gm
...
function AddSetting2(scene, x, y, z, distance)
	local form = util_get_form(THIS_FORM, false, false)
	local game_config = nx_value("game_config")
	local account = game_config.login_account
	local dir = nx_function("ext_get_current_exe_path") .. "ht_auto_" .. account 
	...
	local string = scene .. "," .. x .. "," .. y .. "," .. z .. "," .. distance
	gm_list:AddString(string)
	gm_list:SaveToFile(file)
	LoadSetting2(form)
	AutoSendMessage("Lưu danger point thành công : " .. nx_string(string))
end

function RemoveSetting2(form)
	local game_config = nx_value("game_config")
	local account = game_config.login_account
	local dir = nx_function("ext_get_current_exe_path") .. "ht_auto_" .. account 
	local file = dir .. nx_string("\\auto_donghai_danger.txt")
	local gm_list = nx_create("StringList")
	local removestring = item2[form.select_index]
	item2[form.select_index] = ""

	for k = 1, table.getn(item2) do
		if item[k] ~= "" then
			gm_list:AddString(item2[k])
		end
	end
	AutoSendMessage("Xóa danger point thành công : " .. nx_string(removestring))
	gm_list:SaveToFile(file)
	LoadSetting2(form)
end

function IsInDangerZone(scene, x, y, z)
	local result = false
	for k = 1, table.getn(item2) do
		local pos = util_split_string(nx_string(item2[k]), ",")
		if scene == pos[1] then
			if distance3d(x, y, z, pos[2], pos[3], pos[4]) <= nx_number(pos[5]) then
				result = true
				break
			end
		end
					...
					if not game_client:IsPlayer(object.Ident) and object:QueryProp("Type") == 4 then
						...
						if nx_is_valid(vis_object) then
							if object:FindProp("NpcType") and object:FindProp("GatherRange") then
								...
								if object:QueryProp("NpcType") == 468 and not IsInDangerZone(scene:QueryProp("Resource"), vis_object.PositionX, vis_object.PositionY, vis_object.PositionZ) then
									return object
								end
							end
						end
...
function on_btn3_click(cbtn)
	...
	if nx_is_valid(form) then
		if form.select_index == 0 then
				AutoSendMessage("Vui lòng chọn tọa độ")
				return
		end
		RemoveSetting(form)
	end
	...
	if nx_is_valid(form) then
		...
		if nx_is_valid(game_client) and nx_is_valid(game_visual) then
			...
			if nx_is_valid(scene) then
				  ...
				  local y = string.format("%.3f", visual_player.PositionY)
				  local z = string.format("%.3f", visual_player.PositionZ)
				AddSetting2(scene:QueryProp("Resource"),x,y,z,15)
			end
		end
	end
end
	...
	while form.auto_start and nx_is_valid(form) do
		...
		if nx_is_valid(game_client) and nx_is_valid(game_visual) then
			...
			if scene:QueryProp("Resource") == pos[1] then
				if IsPlayerHaveBuff("buf_ride_yufeng_ready") or IsPlayerHaveBuff("buf_ride_yufeng") then
					if kt_object ~= nil then
						...
						if nx_is_valid(vis_object) then
							if not is_busy() and nx_is_valid(kt_object) then
								...
								if distance <= 2.5 then
									game_visual:SwitchPlayerState(visual_player, "static", 1)
									...
									if not is_busy() and distance2d(visual_player.PositionX, visual_player.PositionZ, start_x, start_z) <= 0 then
										...
										-- TeleportTest(nx_float(start_x), nx_float(start_y) + 1.5, nx_float(start_z))
									end
						...
						if not is_busy() and distance >= 10 then
							...
							if not nx_execute("util_move", "is_path_finding", visual_player) then
								nx_execute("hyperlink_manager", "find_path_npc_item", "findnpc," .. scene:QueryProp("Resource") .. "," .. nx_string(pos[2]) .. "," .. nx_string(pos[4]), 1)
							end
						else
							if form.mode == 0 then
					...
					-- if not IsPlayerHaveBuff("buf_lilianyao_2") then
					-- 	UseItemById("growitem_11")
					-- end
					if not IsPlayerHaveBuff("itmbuf_neixiu_07", 2) then
						UseItemById("itm_neixiu_07")
					end
					UseItemById("ride_windrunner_003", 2)
				end
			else
				if form.mode == 0 then
					if form.cur_point + 1 >= form.total_point then

...
local array_formula = {
	{"hc_1000054", "tool_qingame_startgame1"}, -- Nội lực
	{"hc_1000053", "tool_qingame_startgame2"}, -- Máu
	{"hc_1000056", "tool_qingame_startgame3"} -- Bạo kích
}
...
function auto_init()
	if not nx_execute("form_stage_main\\form_life\\form_job_main_new", "find_life_job", "sh_qs") then
		tools_show_notice(util_text("tool_no_dan"), 1)
		return 0
	end
	...
	if auto_is_running >= 4 then
		auto_is_running = 0
		tools_show_notice(util_text("tool_qingame_stop"))
	end
	...
	if auto_is_running ~= 0 then
		if not nx_execute("form_stage_main\\form_life\\form_job_qishi", "is_Learned_formula", array_formula[auto_is_running][1]) then
			tools_show_notice(util_text("tool_qingame_nodan") .. nx_widestr(" ") .. util_text(array_formula[auto_is_running][2]))
			auto_is_running = 0
			return 0
		end
		
		tools_show_notice(util_text("tool_qingame_startgame") .. nx_widestr(" ") .. util_text(array_formula[auto_is_running][2]))

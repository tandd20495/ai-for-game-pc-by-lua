...
function auto_init()
	...
	if not auto_is_running then
		...
		num_repeated = 0
		tools_show_notice(util_text("tool_auto_captureqt_start"))
		...
		while auto_is_running == true do
			...
			if num_repeated >= 6 then
				num_repeated = 0
				tools_show_notice(util_text("tool_auto_captureqt_running"))
			end
			nx_pause(2)
		end
	else
		auto_is_running = false
		tools_show_notice(util_text("tool_auto_captureqt_end"))
	end
end
...
---add
  ...
  for i = 1, form.grid_row_num do
      for j = 1, form.grid_clomn_num do
        local images = grid:GetItemImage(test)
 ...
 --   timer:UnRegister(nx_current(), "on_update_time", form)
  --  return
          ...
          on_imagegrid_pic_select_changed(grid, test)
          nx_pause(0.5)
          on_imagegrid_pic_select_changed(grid, needtofind - 1)
...
function init_grid(row_num, clomn_num, photo_width, photo_height)
  ...
  for i = 1, row_num do
    for j = 1, clomn_num do
      ...
      console_log("sss " ..i .. " - " .. j .. " -" .. nx_string(init_left) .. "," .. nx_string(init_top))
      local images = grid:GetItemImage(test)
      images = string.gsub(images, "gui\\mainform\\smallgame\\picture_game\\picture\\", "")
      console_log("ddd " .. images)

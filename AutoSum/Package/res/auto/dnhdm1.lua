...
--"moemoe0111|ngoinha09",
--"moemoe0112|ngoinha09",
--"moemoe0113|ngoinha09",
--"moemoe0114|ngoinha09",
--"moemoe0115|ngoinha09",
--"moemoe0116|ngoinha09",
--"moemoe0117|ngoinha09",
"moemoe0118|ngoinha09",
"moemoe0119|ngoinha09",
"moemoe0120|ngoinha09",
}
...
local Hop_HDM = "Box_xdm_taskprize_13"
local Hop_HDM1 = "Box_xdm_taskprize_9"
...
nx_pause(10)
  ...
  nx_pause(0.2)	 
  for j = 1,2 do
		 nx_execute("form_stage_main\\form_bag_func", "use_item_by_configid", Hop_HDM1)
	 ...
	 --add_chat_info(items[i])
       rut_qua(items[i], bagTypes[i])
        arrange_bag(2, 0)
       arrange_bag(bagTypes[i], checkNumbers[i])
     end
	 nx_pause(0.2)	 
		 nx_execute("form_stage_main\\form_bag_func", "use_item_by_configid", Hop_HDM)
	 ...
	 local hasItem, id =  has_item(hd_lenh, 125, 0)
		if hasItem then
		add_chat_info("Gui Thu")
      gui_item_di(".Man..", id, 125)
...
if da_nhan_quest(20991) then
  ...
  if da_nhan_quest(20991) then
	...
	local hoa = "Gather_xdm_dy_zczf1"
	local chi_dan_pos = {-658.445,262.161,-295.744,1.327}
					jump_to(chi_dan_pos)
	nx_pause(5)			
					if getNpcById(hoa) ~= nil then
					jump_to(chi_dan_pos)
  ...
  elseif da_nhan_quest(20992) then
	...
	local hoa = "Gather_xdm_dy_zczf2"
	local chi_dan_pos = {-640.252,273.395,-273.187,-2.544}
					jump_to(chi_dan_pos)
	nx_pause(5)			
					if getNpcById(hoa) ~= nil then
					jump_to(chi_dan_pos)
    ...
    elseif da_nhan_quest(20994) then
	local hoa = "Gather_xdm_dy_zczf4"
	 local chi_dan_pos = {-611.013,286.681,-211.502,1.927}
					jump_to(chi_dan_pos)
	nx_pause(5)			
					if getNpcById(hoa) ~= nil then
...
local chi_dan_pos = {-658.445,262.161,-295.744,1.327}
 ...
 local npcTraQ = "NPC_xdm_dy_hs"
	...
	elseif step == 150 then
	 ...
	 add_chat_info("da vao step 150 nhanh 1")
	  step = 7
	  else 
	  add_chat_info("da vao step 150 nhanh 2")
	  step = 10
	  end
  elseif step == 10 then
  if da_nhan_quest(20996) then
	...
	nx_pause(5)
					...
					if query_task_done_single(20996) then						
					step = 300
					end
					end
...
if query_task_done_single(20997) then								
					step = 11
					end
					end
	...
	nx_pause(5)
					...
					local chi_dan_pos = {-637.666,262.162,-320.510,-0}
					jump_to(chi_dan_pos)
						 nx_pause(0.5)
					step = 11
					end
	...
	local tu_tu_moi_bat = "NPC_xdm_dy_mpzf1"
		...
		if getNpcById(tu_tu_moi_bat) ~= nil then
	 add_chat_info("da vao step 300 nhanh 1")
	  step = 10
	  else 
	  add_chat_info("da vao step 300 nhanh 2")
	  step = 11
	  end
...
if query_task_done_single(20997) then							
	...
	elseif step == 400  then
		...
		if getNpcById(tu_tu_moi_bat) ~= nil then
	 add_chat_info("da vao step 400 nhanh 1")
	  step = 12
	  else 
	  add_chat_info("da vao step 100 nhanh 2")
	  step = 13
	  end

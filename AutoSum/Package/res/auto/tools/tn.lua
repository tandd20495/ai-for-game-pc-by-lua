...
-- function auto_init()
	...
	if not auto_tn then
		...
		if data ~= 1 then
			tools_show_notice(nx_function("ext_utf8_to_widestr", "Đang auto thụ nghiệp"))
			...
			if tools_move_isArrived(data.toa_do.x, data.toa_do.y, data.toa_do.z) and checktime() then
